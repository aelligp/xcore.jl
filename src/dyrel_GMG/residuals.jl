...
@kernel function compute_dyrel_residual_W_kernel!(R_W, @Const(W), @Const(U),
                                                  ...
                                                  @Const(advn_mz), @Const(D_W), @Const(bext_W),
                                                  invh, g0, third, half, a1, a2, a3, inv_dt, dt, fscale,
                                                  dr::Bool)
    ...
    @inbounds begin
        ...
        if iz_f == 1 || iz_f == Nzf
            ...
            Wv       = W[iz_f, ix + 1]
            # τzz / τxz computed INLINE (same formula as dyrel/stress.jl) — no
            # stress kernel, no txx/tzz/txz round-trip. Bit-exact.
            ...
            # R = −(KV·W + GG·P − rr): viscous (+∇·τ) and pressure (−∂P) and the
            # operator-diagonal inertia/sailor (∝ current W) keep their sign and
            # are ALWAYS present. The pure forcing (buoyancy + lagged − adv) is
            # scaled by `fscale` (1 on the fine grid, 0 on coarse error levels),
            # and `bext_W` injects the restricted residual (0 on the fine grid).
            r = d_tzz_dz + d_txz_dx - dP_dz - inertia - sailor +
                fscale * (body + lagged - adv) + bext_W[iz_f, ix]
            if dr
...
@kernel function compute_dyrel_residual_U_kernel!(R_U, @Const(U), @Const(W),
                                                  ...
                                                  @Const(advn_mx), @Const(D_U), @Const(bext_U),
                                                  invh, g0, third, half, a1, a2, a3, gamma, inv_dt, dt, fscale,
                                                  dr::Bool)
    ...
    @inbounds begin
        ...
        ixR = ix == Nx + 1 ? 1  : ix
        Uv       = U[iz + 1, ix]
        # τxx / τxz computed INLINE (same formula as dyrel/stress.jl). Bit-exact.
        ...
        adv      = advn_mx[iz, ix]
        # operator-diagonal inertia/sailor always present; pure forcing
        # (lagged − adv; no buoyancy for U) scaled by `fscale`; `bext_U` injects
        # the restricted residual (0 on the fine grid). See W kernel.
        r = d_txx_dx + d_txz_dz - dP_dx - inertia - sailor +
            fscale * (lagged - adv) + bext_U[iz, ix]
        if dr
...
@kernel function compute_dyrel_residual_P_kernel!(R_P, P_num, @Const(W), @Const(U),
                                                  ...
                                                  @Const(η_b), @Const(bext_P), @Const(γ_eff),
                                                  invh, inv_dt, fscale)
    iz, ix = @index(Global, NTuple)
    @inbounds begin
        ...
        # 1/ρ-scaled continuity (see single-grid note). `fscale` zeroes the MFS
        # source on coarse error levels; `bext_P` injects the restricted
        # continuity residual (already in the 1/ρ-scaled space). div_ρv and comp
        # are operator terms (∝ current v,P) and always present.
        rp = (fscale * MFS[iz, ix] - div_ρv - comp) / rho[iz, ix] + bext_P[iz, ix]
        R_P[iz, ix]   = rp
        # Fused Schur penalty P_num = γ_eff·R_P (was a separate broadcast per DR
        # iter in smoother.jl) — one fewer kernel launch per inner iteration.
...
function dyrel_residual_V!(cache::DyrelCache{T}, phase::PhaseState{T},
                           ...
                           gamma::Real, dt::Real, fscale::Real = 1) where {T<:AbstractFloat}
    backend = KernelAbstractions.get_backend(cache.R_W)
    invh   = T(inv(grid.h));  g0 = T(par.g0)
    inv_dt = isfinite(dt) ? T(inv(dt)) : zero(T)
    dtT    = T(dt);  fs = T(fscale);  third = T(1//3);  half = T(1//2)
    # Stress recomputed inline in the residual kernels (W, U, eta, etaco) — no
    # phase.txx/tzz/txz round-trip, no separate stress kernel.
    compute_dyrel_residual_W_kernel!(backend)(
        cache.R_W, fluid.W, fluid.U, fluid.eta, fluid.etaco, fluid.P, cache.P_num, fluid.Drho,
        fluid.rho, fluid.rhow, fluid.rhoWo, fluid.rhoWoo, cache.advn_mz, cache.D_W, cache.bext_W,
        invh, g0, third, half, T(a1), T(a2), T(a3), inv_dt, dtT, fs, dr; ndrange = size(cache.R_W))
    compute_dyrel_residual_U_kernel!(backend)(
        cache.R_U, fluid.U, fluid.W, fluid.eta, fluid.etaco, fluid.P, cache.P_num,
        fluid.rho, fluid.rhou, fluid.rhoUo, fluid.rhoUoo, cache.advn_mx, cache.D_U, cache.bext_U,
        invh, g0, third, half, T(a1), T(a2), T(a3), T(gamma), inv_dt, dtT, fs, dr; ndrange = size(cache.R_U))
...
function dyrel_residual_P!(cache::DyrelCache{T}, fluid::FluidState{T},
                           ...
                           P0::AbstractMatrix{T}, dt::Real;
                           fscale::Real = 1) where {T<:AbstractFloat}
    ...
    compute_dyrel_residual_P_kernel!(backend)(
        ...
        fluid.rhow, fluid.rhou, cache.η_b, cache.bext_P, cache.γ_eff, invh, inv_dt, T(fscale);
        ndrange = size(cache.R_P))
    # No synchronize: KA keeps same-backend launches ordered; sync happens at
    # the next host read (the reductions / convergence checks).
    return nothing
end
