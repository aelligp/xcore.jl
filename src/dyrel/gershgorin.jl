...
#  - xcore's stress convention τ = η(ε − ⅓∇·v I) (NOT JustRelax's 2η): viscous
#    coefficients are HALVED vs JustRelax (normal cn=2/3, cross cc=1/3, shear
#    cs=1/2) so D is the true Jacobi diagonal of the operator fluidmech.jl
#    assembles (fluidmech.jl:210/285, dyrel/stress.jl). γ_eff penalty keeps
#    coefficient 1.
...
@kernel function compute_dyrel_gershgorin_U_kernel!(D_U, λmax_U, @Const(η), @Const(etaco),
    ...
    @inbounds begin
        ...
        # Gershgorin row sums in xcore's stress convention τ = η(ε − ⅓∇·v I)
        # — NOT JustRelax's 2η. Normal η → cn = 2/3, deviatoric cross η →
        # cc = 1/3, shear η → cs = 1/2: exactly half the 2η coefficients, so D
        # equals the viscous diagonal assembled in fluidmech.jl:285 and
        # dyrel/stress.jl. γ_eff (penalty) keeps coefficient 1 — it is the
        # div-penalty term, not viscous stress, and is injected at γ_eff scale
        # via P_num (MATH.md §5). Here η_E/η_W are cell-centred (normal), η_N/η_S
        # are corner (shear).
...
@kernel function compute_dyrel_gershgorin_W_kernel!(D_W, λmax_W, @Const(η), @Const(etaco),
    ...
    @inbounds begin
        ...
        # xcore convention τ = η(ε − ⅓∇·v I): normal η_N/η_S → cn = 2/3, cross
        # → cc = 1/3, shear η_E/η_W → cs = 1/2 (half of JustRelax's 2η coeffs;
        # matches fluidmech.jl:210 / dyrel stress). γ_eff keeps coefficient 1.
...
function gershgorin!(cache::DyrelCache{T}, fluid::FluidState{T}, h::Real;
    ...
    invh    = T(inv(h))
    invh2   = invh * invh
    # Viscous coefficients in xcore's τ = η(ε − ⅓∇·v) convention (half the 2η
    # values JustRelax uses): cn normal, cc deviatoric cross, cs shear.
    cn      = T(2//3)
    cc      = T(1//3)
    cs      = T(1//2)
